...
setwd("C:/Users/wangha/Desktop/AhR project/Suspect screening analysis/09_Thrid_round_of_identification_all_samples_update_ML_predictions/07_Alignment_Witten")
...
targetlist <- df$`Unknown Id in Wittenberg`
Wittenberg <- msp[[7]]
result <- Filter(function(x) x$UnknownID %in% targetlist, Wittenberg$msp_objs)
...
colnames(Summary_ID)[colnames(Summary_ID) == "innermost_name"] <- "Wittenberg"



# Create a mapping from 'Unknown Id in Schmilka' to 'name' in df
name_mapping <- setNames(df$name, df$`Unknown Id in Wittenberg`)

# Map the IDs in Summary_ID to get the corresponding names
Summary_ID$Chemicals <- name_mapping[as.character(Summary_ID$Wittenberg)]
...
# Rename 'innermost_name' to just 'innermost_name' or keep it
colnames(Summary_area)[colnames(Summary_area) == "innermost_name"] <- "Wittenberg"

# Map the IDs in Summary_ID to get the corresponding names
Summary_area$Chemicals <- name_mapping[as.character(Summary_ID$Wittenberg)]
...
#For method detection limit (MDL) and method quantification limit (MQL)
# If there is a detection in procedural blank, then MDL = average + 3*std (n=2 for this study), MQL = average + 10*std
# if there is no detection in procedural blank, then MDL = the smallest CAL, MQL = Intercept/slope if intercept is a positive value



#df_summary_unknown$Average_PB <- (df_summary_unknown$18_Procedural_Blank_1_calc + df_summary_unknown$29_Procedural_Blank_2_calc) / 2
