...
setwd("C:/Users/wangha/Desktop/AhR project/Suspect screening analysis/09_Third_round_of_identification_all_samples_update_ML_predictions/11_Alignment_confirm_standard")
...
targetlist <- df$`Unknown Id in 950 ppb`
CAL950 <- msp[[1]]
result <- Filter(function(x) x$UnknownID %in% targetlist, CAL950$msp_objs)
  ...
  for (j in seq_along(result)) {
    ...
    matches <- Filter(function(x) {
      ...
      abs(modelion_num - target_model_ion) <= 0.005 &&
        abs(retentiontime - target_retention_time) <= 0.04
    }, all_elements)
...
colnames(Summary_ID)[colnames(Summary_ID) == "innermost_name"] <- "CAL950"



# Create a mapping from 'Unknown Id in Schmilka' to 'name' in df
name_mapping <- setNames(df$name, df$`Unknown Id in 950 ppb`)

# Map the IDs in Summary_ID to get the corresponding names
Summary_ID$Chemicals <- name_mapping[as.character(Summary_ID$CAL950)]
...
# Rename 'innermost_name' to just 'innermost_name' or keep it
colnames(Summary_area)[colnames(Summary_area) == "innermost_name"] <- "CAL950"

# Map the IDs in Summary_ID to get the corresponding names
Summary_area$Chemicals <- name_mapping[as.character(Summary_ID$CAL950)]
...
first_group <- head(Chemical_names_filtered, 13)
last_group  <- tail(Chemical_names_filtered, 3)
...
for (i in 1:nrow(Summary_area)) {
  ...
  if (chem_name %in% first_group) {
    ...
    # For all other chemicals, use ref_Acenaphthene
...
#-------------------------------------------------------------------------------
write.csv(df_coeffs, file = "slope_confirmed_standards.csv", row.names = FALSE)